...
## 当前阶段：Fast IR-MAD 单对比较

仅测试 `20250118 → 20260203`。复用 Full，Fast 只在 IR-MAD 迭代阶段从全部有效像元序列等分区间确定性取中点，最多 4,000,000 像元。最终 NCP/PIF 和 TLS 仍使用全量有效像元，沿用相同 TIFF 写出函数。此阶段不运行道路提取或变化检测。

入口为 `fast_irmad_compare.py`（已有输出时拒绝重跑），结果位于 `irmad/fast_4m/REPORT.md` 和 `comparison.json`。本对输出 MAE 0.372 DN、RMSE 0.625 DN，拟合耗时 134.02 → 13.66 秒；Fast 总耗时 140.17 秒。Full 历史总耗时 313.25 秒包含导出修复，不能作为严格端到端加速比。Fast 可作为后续验证候选，尚未接入正式流程。数据和报告继续由本目录 `.gitignore` 排除，仅代码和本说明纳入 Git。
