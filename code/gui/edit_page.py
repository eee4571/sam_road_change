...
class EditPage:
    ...
    def _build_review_page(self, page: ttk.Frame) -> None:
        ...
        selector.pack(fill=X, pady=(0, LAYOUT_METRICS["form_gap"]))
        ...
        edit_row.pack(fill=X, pady=LAYOUT_METRICS["form_gap"])
        ...
        review_detail_label.pack(anchor="w", fill=X, pady=(LAYOUT_METRICS["form_gap"], LAYOUT_METRICS["module_gap"]))
        ...
        self.apply_review_button.pack(side=LEFT, padx=LAYOUT_METRICS["module_gap"])
        self.review_advanced_toggle = ttk.Button(review_card, text="高级操作...", command=self._toggle_review_advanced)
        self.review_advanced_toggle.pack(anchor="w", pady=(LAYOUT_METRICS["module_gap"], 0))
        ...
        ttk.Label(summary, text="当前对象：", width=12, style="Metric.TLabel").grid(row=0, column=0, sticky="nw", pady=LAYOUT_METRICS["form_gap"] // 2)
