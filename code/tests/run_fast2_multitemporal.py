...
def main():
    ...
    previous=args.job/'_profiling'/'fast2_object_reconciliation'
    output=args.job/'_profiling'/'fast2_multitemporal';output.mkdir(exist_ok=True,parents=True)
    ...
    for pair in manifest['change_results']:
        ...
        started=time.perf_counter();scenes=[]
        ...
        try:
            ...
            context_seconds=time.perf_counter()-loaded;presence=[];tick=time.perf_counter()
            with patch.object(legacy,'analyze_scenes',side_effect=AssertionError('Fast1 fallback')), \
                ...
                result=analyze_scenes(*scenes,tolerance=float(pair['tolerance']),absolute=float(pair['absolute']),
                                     relative=float(pair['ratio']),presence_audit=presence,temporal_context=context)
            ...
            row=dict(pair=key,before=baseline[key]['after'],after=assess(current,truth,area),
                ...
                total_seconds=loaded-started+context_seconds+analysis+export,counts=result[3],
