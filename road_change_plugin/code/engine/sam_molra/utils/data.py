...
def default_loader(id, root, size = (512, 512)):
    ...
    mask_path = os.path.join(root, '{}.png'.format(id))
    if not os.path.exists(mask_path) and id.endswith('_sat'):
        mask_path = os.path.join(root, '{}_mask.png'.format(id[:-4]))
    mask = cv2.imread(mask_path, cv2.IMREAD_GRAYSCALE)
    if img is None:
        raise FileNotFoundError(os.path.join(root, '{}.jpg'.format(id)))
    if mask is None:
